...
const SubmittedApplicationCard: React.FC<Props> = ({ item, onUpdated }) => {
  ...
  const [editingFileComment, setEditingFileComment] = useState<number | null>(
    null
  );
  ...
  const handleSaveMainComment = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `https://tanlov.medsfera.uz/api/student/application-items/${item.id}/`,
        {
          method: "PATCH",
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.detail || "Asosiy izohni saqlashda xatolik yuz berdi"
        );
      }
  ...
  const handleSaveFileComment = async (fileId: number) => {
    ...
    try {
      const res = await fetchWithAuth(
        `https://tanlov.medsfera.uz/api/student/files/${fileId}/`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.detail || "Fayl izohini saqlashda xatolik yuz berdi"
        );
      }
  ...
  const handleFileReplace = async (fileId: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.detail || "Faylni almashtirishda xatolik yuz berdi"
        );
      }
  ...
  const handleUploadNewFile = async () => {
    ...
    try {
      ...
      const res = await fetchWithAuth(
        `https://tanlov.medsfera.uz/api/student/application-items/${item.id}/files/`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.detail ||
            `Faylni yuklashda xatolik yuz berdi (Status: ${res.status})`
        );
      }
      ...
      >
        <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            {(item.title === "Kitobxonlik madaniyati" ||
              item.title === "Talabaning akademik o‘zlashtirishi") && (
              <>
                {item.test_result !== null && item.test_result !== undefined ? (
                  <Typography>
                    {(item.test_result * 20) / 100} ball (Test). Test natijasi:{" "}
                    {item.test_result}%
                  </Typography>
          ...
          {item.reviewer_comment && (
            ...
            >
              ...
              <Typography
                variant="body2"
                sx={{ lineHeight: 1.5, color: "#333333" }}
              >
                ...
                {item.files.length === 0 ? (
                  <Fade in={true}>
                    ...
                    >
                      <Typography
                        variant="caption"
                        sx={{ mb: 1, color: "#666666", display: "block" }}
                      >
                        Fayl yuklash:
                      </Typography>
                      <input
                        ...
                        {editingFileComment === file.id ? (
                          <Fade in={true}>
                            <Box sx={{ mt: 1, pl: isMobile ? 0 : 3 }}>
                              <TextField
                                ...
                                onChange={(e) =>
                                  setFileCommentInput(e.target.value)
                                }
                                sx={{
                                  mb: 1,
                                  "& .MuiOutlinedInput-root": {
                          ...
                          >
                            <Typography
                              ...
                              color="#666666"
                              sx={{
                                fontStyle: file.comment ? "normal" : "italic",
                                flexGrow: 1,
                              }}
                            >
                              {file.comment
          ...
          {!editingMainComment ? (
            <Fade in={true}>
              <Box>
                <Typography
                  variant="body2"
                  sx={{ mb: 1, color: "#333333", lineHeight: 1.5 }}
                >
                  {mainComment || (
                    <em style={{ color: "#999999" }}>Izoh kiritilmagan</em>
                  )}
                </Typography>
                <Button
                  size="small"
